...
#       """Reads a JSON file and returns a map variable."""
    ...
    def getStageSql(self, ProjectId,SourceDatasetId, TableName, IsIncremental, CDCColumnName, CdcUpdateDate,):
    ...
    def getSourceTablePrimaryKeys(self, ProjectId,SourceDatasetId,TargetDatasetId,TableName,PrimaryKeys):
        query = f"""
    ...
    def generate_merge_statement(self, project_id,source_dataset, table_name, target_dataset, primary_key,source_system_id, IsIncremental, soft_delete_column, connection_id):
            ...
            if IsIncremental == 'N':
                ...
                MERGE `{project_id}.{target_dataset}.TEMP_{table_name}` AS target
                        ...
                        WHEN NOT MATCHED THEN
                            INSERT ({', '.join([column_name for column_name,data_type in columns])}, UpdateBy, UpdateDate, UpdateProcess, CreateBy, CreateDate, CreateProcess,LoadBy, LoadDate, LoadProcess, SystemId, InactiveInd, InactiveReason)
                            VALUES ({', '.join([f'DATETIME(source.{column_name})' if data_type == 'DATETIME' else f'source.{column_name}' for column_name, data_type in columns])}, 'insert_airflow', current_timestamp,'insert_airflow', 'insert_airflow', current_timestamp, 'insert_airflow', 'insert_airflow', current_timestamp, 'insert_airflow', {source_system_id}, {'case when source.' + soft_delete_column + "='Y' then FALSE else TRUE end" if soft_delete_column is not None else 'FALSE'},' ');
            ...
            """
                ...
                USING `{project_id}.{source_dataset}.{table_name}` AS source
                ON {' AND '.join([f'target.{pk} = {"DATETIME(source." + pk + ")" if any(data_type == "DATETIME" for column_name, data_type in primary_key_cols if column_name == pk) else "source." + pk}' for pk in primary_key])}
                AND target.SystemId = {source_system_id}
                WHEN MATCHED THEN
                    UPDATE SET {', '.join([f"target.{column_name} = {'DATETIME(source.' + column_name + ')' if data_type == 'DATETIME' else 'source.' + column_name}" for column_name, data_type in columns if column_name not in primary_key])}
                ...
                WHEN NOT MATCHED THEN
                    INSERT ({', '.join([column_name for column_name,data_type in columns])}, UpdateBy, UpdateDate, UpdateProcess, CreateBy, CreateDate, CreateProcess,LoadBy, LoadDate, LoadProcess, SystemId, InactiveInd, InactiveReason)
                    VALUES ({', '.join([f'DATETIME(source.{column_name})' if data_type == 'DATETIME' else f'source.{column_name}' for column_name, data_type in columns])}, 'insert_airflow', current_timestamp,'insert_airflow', 'insert_airflow', current_timestamp, 'insert_airflow', 'insert_airflow', current_timestamp, 'insert_airflow', {source_system_id},FALSE,' ')
            """
    ...
    def get_cdc_load_date(self,source_system_id, project_id, dataset_id, table_name, connection_id):
        ...
        query = f"""
                SELECT CdcLoadDate
                FROM `{project_id}.Ods.CdcLoadDetails`
    ...
    def upsert_cdc_load_details(self,source_system_id, project_id, dataset_id, table_name, cdc_load_date,load_execution_status):
        """
    ...
    def update_InactiveInd(self,project_id,connection_id, source_dataset, target_dataset,  table_name, primary_key, archive_date, archive_date_column):
        now= datetime.now()
        ...
        query = f"""
            ...
            WHERE 
            {' AND '.join([f'target.{pk} = {"DATETIME(source." + pk + ")" if any(data_type == "DATETIME" for column_name, data_type in primary_key_cols if column_name == pk) else "source." + pk}' for pk in primary_key])})
            """
