...
configurations_path = "/home/airflow/gcs/dags/{}/Taleo/Configurations/".format(Environment)
common_var_path = "/home/airflow/gcs/dags/{}/Taleo/Configurations/Common/".format(Environment)
common_var = common_var_path + "common_variables.json" #"Common_Variables.txt"
dag_var_path = "/home/airflow/gcs/dags/{}/Taleo/Configurations/{}/Variables/".format(Environment,Dag_Name)
...
dataform_Tags = dag_variables.get("DF_Tags_List")
...
# Iterate over the list of Dataform DAGs
for dataform_Tag in dataform_Tags:
    
    # Define the DAG ID for each Dataform DAG
    dag_id = f"{Environment}_{dataform_Tag}"
    

    # task to create dataform compilation result for each DAG
    create_compilation_result = DataformCreateCompilationResultOperator(
        task_id=f"create_compilation_result_{dataform_Tag}",
    ...
    # task to create dataform workflow invocation for each DAG
    create_workflow_invocation = DataformCreateWorkflowInvocationOperator(
        task_id=f'create_workflow_invocation_{dataform_Tag}',
        project_id=common_variables['Project'],
        region=common_variables['DF_Region'],
        repository_id=common_variables['DF_Repository_Id'],
        workflow_invocation={
            "compilation_result": "{{ task_instance.xcom_pull('create_compilation_result_" + dataform_Tag+ "')['name'] }}",
            "invocation_config": { "included_tags": {dataform_Tag} , "transitive_dependencies_included": True }
        },
        gcp_conn_id=GCP_CONN_ID,
    )
