...
def on_failure_callback(context):
    try:
        ...
        if runauditid is not None:
            Run_Audit_module.update_audit_info(arg1={'RunAuditId': runauditid, 'Status':"Failed",'Reason': reason})
...
with DAG(
 ...
 # Task to get the result of execute_sp_GetRunAuditInfo
    Task_GetRunAudit = PythonOperator(
        task_id="GetRunaudit", python_callable=Run_Audit_module.get_runid_timeframe,
        provide_context=True,
        dag=dag,
    )
